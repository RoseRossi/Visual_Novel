...
const { uriMongo } = require('./db.js');

// Simulamos la carga de variables de entorno para las pruebas
jest.mock('dotenv', () => ({ config: jest.fn() }));
config();

describe('Configuración', () => {
  it('debería exportar la URI de MongoDB', () => {
    // Simulamos el valor de la variable de entorno
    process.env.URIMOONGODB = 'mocked-uri';

    // Requerimos nuevamente para que tome el valor simulado
    const { uriMongo: uriMongoMocked } = require('./tuArchivoConConfig');

    // Verificamos que el valor exportado sea el esperado
    expect(uriMongoMocked).toBe('mocked-uri');
  });
});
